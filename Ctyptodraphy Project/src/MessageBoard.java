         ...
         if (clientsCounter < ClientsArray.length)       
            {
                ...
                for (int i=0; clientsCounter<ClientsArray.length; i++)           // a loop that broadcasts to everyone that this user is now connected
                {
                    ...
                    {
                    	System.out.println(((ConnectionProxy)ClientsArray[clientsCounter]).getName()+"hereeeeee");
                        ((ConnectionProxy)ClientsArray[i]).consume(((ConnectionProxy)ClientsArray[clientsCounter]).getName()); //confirming connection
